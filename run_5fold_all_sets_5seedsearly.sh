...
for meta_dir in "${META_SETS[@]}"; do
  ...
  meta_log_dir="logs/5run_5fold_early/${meta_dir}"
...

  {
    ...
    # for run_idx in 1; do
      ...
      for fold in 1 2 3 4 5; do
        checkpoint_dir="/data/storage1t/projects/early/BDHPD/${meta_dir}/run_${run_idx}/fold_${fold}"
        ...
        CUDA_VISIBLE_DEVICES=${cuda_device} python train.py --config "${config_file}" \
          ...
          --Neurovoz_and_PC_GITA.train_metadata_path="${full_meta_root}/fold_${fold}/sub_splits/train_earlybalance.tsv" \
